...
.simplelineicons-demo .item-box .item.selected{
	background: #eee;
}
</style>
</head>
...
<div class="yy-toolbar">
	<button id="yy-btn-select" class="btn blue btn-sm">
		<i class="fa fa-check"></i> 确定
	</button>
	<button id="yy-btn-clear" class="btn blue btn-sm">
...
function selectedIcon(obj){
	var iconClass;
	iconClass= $("i",obj).attr("class");
	if(iconClass==null||iconClass==''){
		iconClass= $("span.glyphicon",obj).attr("class");
	}
	if(iconClass==null||iconClass==''){
		iconClass= $("span:not(.glyphicon)",obj).attr("class");
	}
	//回调行数
	window.parent.callBackSelectIcon(iconClass);
	var index = parent.layer.getFrameIndex(window.name); //先得到当前iframe层的索引
	parent.layer.close(index); //再执行关闭 
}

$(document).ready(function() {
	$("div.fa-item,.bs-glyphicons-list li,span.item").bind("dblclick",function(){
		selectedIcon(this);
	});
	$("div.fa-item,.bs-glyphicons-list li,span.item").bind("click",function(){
		_iconObj = this;
		$(".selected").removeClass("selected");
		$(_iconObj).addClass("selected");
	});
	
	$("#yy-btn-select").bind("click",function(){
		if(_iconObj==null){
			YYUI.promMsg("请选择图标");
			return false;
		}
		selectedIcon(_iconObj);
	});
	$("#yy-btn-clear").bind("click",function(){
		window.parent.callBackSelectIcon("");
