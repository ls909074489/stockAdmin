...
<body>
	...
	<div>
		<form id="yy-form-edit" class="form-horizontal yy-form-edit">
			<input name="uuid" id="uuid" type="text" class="hide" value="${uuid}">
			...
			<table style="width: 98%;text-align: center;">
				<tr>
					...
					<td style="float: left;">
						当前订单未交付数:<span id="notDoneSpan"></span>,可交付数:<span id="hasDoneSpan"></span>	
					</td>
				</tr>		
				<tr>
					<td style="width: 18%;">
					...
					<td>
						<input name="deliveryNumber" type="text" class="form-control" value="">
					</td>
					<td style="width: 18%;">
						交付数量
					</td>
					<td>
						<input name="deliveryCount" type="text" class="form-control" value="">
					</td>
				</tr>			
			</table>
		</form>
	...
	<script type="text/javascript">
		...
		function setValue(id) {
			var rowData=window.parent.getRowData(id);
			//(列表中已有的deliveryCount-planCount)
			$("#notDoneSpan").html(rowData.planCount-rowData.deliveryCount);
			...
			$.ajax({
				...
				success : function(data) {
					console.info(data);
					if (data.flag==0) {
						$("#hasDoneSpan").html(data.obj.existCount);
					}
				},
				error : function(data) {
		...
		function validateForms(){
			validata = $('#yy-form-edit').validate({
				onsubmit : true,
				rules : {
					'deliveryNumber' : {required : true,maxlength : 100},
					'deliveryCount' : {required : true,number:true,digits:true,maxlength : 20}
				}
			});
		}
		...
		function onSave(isClose) {
			...
			var posturl = "${apiurl}/order/delivery";
			var opt = {
				url : posturl,
				type : "post",
				xhrFields: {withCredentials: true},
