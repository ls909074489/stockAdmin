...
<body class="page-sidebar-closed">
		...
		<div class="page-header-inner ">
			...
			<div class="page-logo" style="width: 650px;">
				<c:if test="${!empty yy_logo_imge}">
					<a href="#"><img src="${ctx}${yy_logo_imge}" alt="logo" class="logo-default" style="width: 30px;height: 30px;"/> </a>
				</c:if>
					...
					<li class="dropdown dropdown-user">
						<a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown"
							data-hover="dropdown" data-close-others="true"> <img alt="" class="img-circle"
								...
								<span class="username username-hide-on-mobile"> ${user.username}
		...
		<div class="page-content-wrapper">
			...
			<div class="page-content">
				...
				<div class="content-tabs page-bar">
					...
					<nav class="page-tabs J_menuTabs">
						<div class="page-tabs-content">
							<a href="javascript:;" class="active J_menuTab" data-id="${indexEnum.enumdatakey}">${indexEnum.enumdataname}</a>
						</div>
					</nav>
				...
				<div id="content-main" class="J_mainContent mainrow">
					<iframe class="J_iframe" id="desktopFrame" name="desktopFrame" width="100%" height="100%" src="${ctx}${indexEnum.icon}"
						data-id="${indexEnum.enumdatakey}" frameborder="0"></iframe>
				</div>
				<!-- end PAGE tabs-->
			</div>
			<!-- END CONTENT BODY -->
	...
	<script>
			...
			layer.confirm('确定退出登录？', function() {
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (data.flag==0||data.flag==-10) {
							window.location = '${ctx}/logout';
						}else {
							YYUI.promAlert("退出登录失败，请联系管理员");
						}
					},
					error : function(data) {
						layer.close(waitInfoLoading);
		...
		$(document).ready(function() {
            ...
            //一小时钟后更新页面
            setInterval ("showNewMsg('0')", 3600000 );
            showNewMsg('1');
		});
		...
		//发送新消息提醒
		function showNewMsg(isNow) {
			  var now = new Date();
		      var hh = now.getHours();            //时
		     if(hh==9||hh==11||hh==14){
		    	 console.info("currrent time");
		    	 isNow=1;
		     } 
		     if(isNow==1){
		    	 var waitInfoLoading = layer.load(2);
					$.ajax({
						type : "POST",
						data :{},
						url : "${apiurl}/sample/getTimeoutCount",
						async : true,
						dataType : "json",
						xhrFields: {withCredentials: true},
				        crossDomain: true,
