...
<div id="${id}" class="page-area">
	...
	<div class="">
		<table id="yy-table-list-file" class="" style="">
		</table>
	</div>
...
　　　　			_fileTableList.draw();
			...
			success : function(data) {
				if(data.records!=null&&data.records.length>0){
					...
					for(var i=0;i<data.records.length;i++){
						fileStr+='<tr><td style="width:80px;text-align:center;">'+
						...
						'<i class="fa fa-trash-o"></i></button></td><td>'+renderViewCol(data.records[i])+'</td></tr>';
						
						//$("#yy-table-list-file").append(fileStr);
						//点击附件名称事件
						/* $(".pic-file-span").on("click", function() {
							showFilePic(this);
						}); */
						console.info(data.records[i]);
						
					}
					$("#yy-table-list-file").html(fileStr);
	...
	function addRow(file){
		...
		var addFileStr='<tr><td style="width:80px;text-align:center;">'+
		//'<button id="yy-btn-downloadfile-row" class="btn btn-xs btn-info downloadfile" data-rel="tooltip" title="下载">'+
		//'<i class="fa fa-download"></i></button>'+
		'<button id="yy-btn-removefile-row"  class="btn btn-xs btn-danger delete removefile" data-rel="tooltip" title="删除">'+
		'<i class="fa fa-trash-o"></i></button></td><td>'+renderViewCol(newData[0])+'</td></tr>';
		
		$("#yy-table-list-file").append(addFileStr);
		setTrAction();//设置行点击事件
		//将要上传的文件保存
		_files.push(file);
	}
	...
	function removeFileRow(nRow){
		console.info(nRow);
		var tempFileSpan=$(nRow).find(".pic-file-span");
		console.info($(tempFileSpan).attr("fileuuid"));
	...
	function setTrAction(){
		$("#yy-table-list-file").find("tr").each(function (){
			console.info($(this));
