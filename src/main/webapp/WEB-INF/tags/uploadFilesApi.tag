...
function FileUploadTool(id,entityType){
	...
	var _fileSize=10485760;//10485760;//上传文件的大小默认10m
	...
	function renderViewCol(data, type, full) {
		...
		if(full.uuid!=null&&full.uuid!=''){
			return "<a target='_blank' href='"+data+"'>"+data+"</a><input class='uploadFilePathCls' type='hidden' value='"+data+"'>";
		}else{
			return "<a target='_blank' href='"+data+"'>"+data+"</a><input class='' type='hidden' value='"+data+"'>";
		}
	...
	function showFilePic(data, iDataIndex, nRow){
		...
		if(data.uploadDate=='未上传'){
			...
			layer.open({
				...
				shade : 0.8,
				area : [ '90%', '90%' ],
				content : '<img width="100%" height="100%" src="'+data.fileName+'"  alt="'+data.fileName+'" />' //iframe的url
			});
		}
	}
	...
	this.loadFilsTableList=function(fileData,editType){
...
　　　　			_fileTableList.draw();
		...
		//加载文件列表 
		_fileTableList.clear();
		_fileTableList.rows.add(records);
		try {
    　　　　		_fileTableList.draw();	
	        　} catch(err) {
	        //YYUI.failMsg("加载数据错误");
	...
	this.saveFiles =function(uploadUrl,fnCallback) {
	        ...
	        $.ajax( {
				...
				success : function(json) {
					if(json.flag==0){
						 _files=new Array();
					     _deleteFiles=new Array();
					     afterUploadFile(json);
					}else if (json.flag==-10) {
						YYUI.promMsg('会话超时，请重新的登录!');
						window.location = '${ctx}/logout';
					}else{
						YYUI.failMsg("上传附件失败：" + json.msg);
					}
				}
			});
	    }else{ 
	    	afterUploadFile({flag: 0, msg: "操作成功", obj: {}, objString: null});
	    }
	}
