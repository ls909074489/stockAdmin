...
function FileUploadTool(id,entityType){
					...
					if(file.size>_fileSize){//10485760
			            ...
			            if(isExitFileType=='1'){
			            	...
			            	if(uploadFileCount!=null&&uploadFileCount>(_vfileCount-1)){//判断上传文件的个数
								...
								+'<button id="yy-btn-removefile-row" style="float:right;" class="btn btn-xs btn-danger delete" data-rel="tooltip" title="删除"><i class="fa fa-trash-o"></i></button>'
								// onclick="removeOssUpFile(this,1);"
			...
			UploadComplete:function(up,files){
				...
				if(files!=null&&files.length>0){
					...
					for(var i=0;i<files.length;i++){
						...
						addFilePaths.push(get_uploaded_object_name(files[i].name)); */
	...
	function renderViewCol(data) {
        ...
        if(ext=='JPG'||ext=='JPEG'||ext=='PNG'||ext=='BMP'){
        	return '<a title="'+data.fileName+'"  style="text-decoration:none;"><span id="pic-file-span" class="pic-file-span" fileuuid="'+data.uuid+'" filepreurl="'+data.preurl+'" fileurl="'+data.url+'" attaType="'+data.attaType+'">'+t_fileName+'</span></a>';
        }
		return '<a title="'+data.fileName+'" style="text-decoration:none;">'+t_fileName+'</a>';
	}
	
	function showFilePic(nRow){
		var spanObj=$(nRow).find(".pic-file-span");;
		var file=uploader.getFile($(nRow).attr("id"));  
		if($(nRow).attr("isadd")=='1'){
			//建立一個可存取到該file的url
				var url = null ; 
				try{
			...
			success : function(data) {
				if(data.records!=null&&data.records.length>0){
					...
					for(var i=0;i<data.records.length;i++){
						...
						fileStr+='<div id="' + data.records[i].uuid + '"  isAdd="0" style="display: inline-block;float:left;padding-right:10px;">' + renderViewCol(data.records[i]) + ' (' + data.records[i].fileSize  + ')<b></b>'
						+'&nbsp;<button id="yy-btn-downloadfile-row" class="btn btn-xs btn-info downloadfile" data-rel="tooltip" title="下载"  type="button">'
						+'<i class="fa fa-download"></i></button>&nbsp;'
						+'<button id="yy-btn-removefile-row" style="float:right;" class="btn btn-xs btn-danger delete" data-rel="tooltip" title="删除" type="button"><i class="fa fa-trash-o"></i></button>'
						+'</div>';
					}
					//$("#yy-upload-file-div").html(fileStr);
					document.getElementById('ossfile').innerHTML =fileStr;
					setTrAction();//设置行点击事件
				...
				if(editType=='edit'){
					$("#ossfile").find(".delete").each(function(){
						$(this).show();
					});
					//$("#"+id+" #yy-upload-file-div .delete").show();
				}else{
	...
	this.removeFileApi = function(nRow) {
		if($(nRow).attr("isAdd")=='0'){//已上传的附件
        	_deleteFiles.push($(nRow).attr("id"));
        	$(nRow).remove();
        }else{
        	uploader.removeFile($(nRow).attr("id"));
        	$(nRow).remove();
        }
	}
