	...
	public void exportQuery(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (explorerType == null || explorerType.contains("IE")) {// IE
				...
				"attachment; filename=\"" + RequestUtil.encode(("库存明细"+DateUtil.getDateTimeZone()),"utf-8")+".xlsx" + "\"");
			} else {// fireFox/Chrome
				response.setHeader("Content-Disposition",
						"attachment; filename=" + new String(("库存明细"+DateUtil.getDateTimeZone()).getBytes("utf-8"), "ISO8859-1")+".xlsx");
			}
