package com.king.common.ehcache;
...
public class YYEhcacheUtil {
	...
	static {
		YYEhcacheUtil.initCacheManager();
		YYEhcacheUtil.initCache("cache");
	}
	...
	public static Cache initCache(String cacheName, long timeToLiveSeconds) throws Exception {
		return YYEhcacheUtil.initCache(cacheName, EHCacheConfig.MAXELEMENTSINMEMORY, EHCacheConfig.OVERFLOWTODISK,
	...
	public static Cache initMyCache(String cacheName) throws Exception {
		return YYEhcacheUtil.initCache(cacheName, EHCacheConfig.TIMETOlIVESECONDS);
	}
	...
	public static boolean setValue(String cacheName, String key, Object value, Integer timeToLiveSeconds)
		...
		try {
			...
			myCache.put(
					new Element(key, value, EHCacheConfig.ETERNAL, EHCacheConfig.TIMETOIDLESECONDS, timeToLiveSeconds));
			return true;
		} catch (Exception e) {
			throw new Exception("set cache " + cacheName + " failed!!!");
		}
	...
	public static void main(String[] arg) {
		...
		YYEhcacheUtil.initCacheManager();
		YYEhcacheUtil.initCache("cache");
		YYEhcacheUtil.put("A", "AAAAA");
		YYEhcacheUtil.put("B", "BBBBB");
		YYEhcacheUtil.put("F", "FFFFF");
		System.out.println(YYEhcacheUtil.get("F"));
		List keys = YYEhcacheUtil.getKeys();
		for (int i = 0; i < keys.size(); i++) {
			System.out.println(keys.get(i));
		}
		YYEhcacheUtil.shutdown();
	}
}
